...
describe('ListInvestimentsService', () => {
  ...
  const URL = 'https://raw.githubusercontent.com/landernodias/fake-server-angular-tests/main/banks.json';
  ...
  it(`(U) should be list all investiments`, (done) => {
    service.list().subscribe(
      ...
      const req = httpTestingController.expectOne(URL); // espera que a url seja igual a URL
      req.flush(mockList);

      expect(req.request.method).toEqual('GET'); // testa o metodo
  });
});
